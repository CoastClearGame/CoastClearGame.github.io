...
void setup(){
  size(384, 512);
  player = new Sprite(this, "Images\\Player.png", 2, 1, 100);
  player.setXY(width/2, height - player.getWidth()/2);
  
  timeCheck = sw.getElapsedTime();
  ...
  batteryPack.setVelY(256);
  batteryPack.setVisible(false);
  warning.setVisible(false);
  wave.setVisible(false);
  wave.setVelXY(128, 256);
  ...
  background(255);
   ...
   if(gameState == 0){
     ...
     if(keyPressed && key == ENTER && keyReset){
       ...
       for(int i = 0; i < 9; i++){
         ...
         forest[i].setVelY(256);
       }
       for(int i = 0; i < 2; i++){
         for(int j = 0; j < 17; j++){
            lanes[i + 2*j].setXY(144 + i * 96, -32 + j * 32);
            lanes[i + 2*j].setVelY(256);
         }
       }
     }
   }
   if(gameState == 1){
     ...
     if(batteryPack.pp_collision(player)){
       batteryLife += batteryLife + 50 > 120 ? 120 - batteryLife : 50;
       batteryPack.setVisible(false);
     }
  ...
  if(obstacleDifference - timeSinceLastObstacle == nextObstacleTime){
    ...
    if(obstacleChoice == 0){
      ...
      for(int i = ii; i < 5; i++){
        ...
        obstacles.get(obstacles.size() - 1).setVelY(256);
        timeSinceLastObstacle -= 1;
